...
/*   Updated: 2022/12/19 13:59:35 by younhwan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	p_take_forks(t_philo *const philo)
{
	...
	printf(FORK_MSG, diff_time(philo->info->started), philo->id);
	pthread_mutex_unlock(philo->info->print_mutex);
	if (philo->info->num_of_philo == 1)
		return (pthread_mutex_unlock(forks[left].mutex));
	if (philo->info->num_of_philo != 1)
...
{
	gettimeofday(philo->last_eat, NULL);
	printf(EATING_MSG, diff_time(philo->info->started), philo->id);
	philo->num_of_eat++;
	pthread_mutex_unlock(philo->info->print_mutex);
...
static t_bool	p_sleep(t_philo *const philo)
{
	...
	if (philo->info->someone_is_dead)
		return (pthread_mutex_unlock(philo->info->print_mutex));
	printf(SLEEPING_MSG, diff_time(philo->info->started), philo->id);
	pthread_mutex_unlock(philo->info->print_mutex);
	...
	if (philo->info->someone_is_dead)
		return (pthread_mutex_unlock(philo->info->print_mutex));
	printf(THINKING_MSG, diff_time(philo->info->started), philo->id);
	pthread_mutex_unlock(philo->info->print_mutex);
	return (TRUE);
}
